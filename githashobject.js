...
const { slice, drop, join } = require("ramda");
...
const createObject = (path, data) => {
  fse.writeFile(path, data, err => {
    if (err) {
      ...
      console.log(
        chalk.green(
          "Your file has been commited " + join("", drop(2, path.split("/")))
        )
      );
    }
  });
...
const createFirstTwoCharFolder = (hashVal, cleanData) => {
  ...
  fse
    .exists(`.git/objects/${firstTwoChar}`)
    .then(fileExist => {
      if (!fileExist) {
        fse.mkdir(`.git/objects/${firstTwoChar}`).then(() => {
          createObject(
            `.git/objects/${firstTwoChar}/${drop(2, hashVal)}`,
            cleanData
          );
        });
      } else {
 ...
 */
exports.hashObject = (file, write = true, data = "") => {
